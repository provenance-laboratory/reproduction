...
```
paper-b-review.zip     17 files, sha256 76e15b9fa17624eccf6595735e7c7888
this file
```

Repository `provenance-laboratory/reproduction`, commit `b7d7f0c`  ⚠️ TREE DIRTY.
...
```
m1  cost of determinism   +37.0%   11 interleaved reps, ranges do not overlap
...
- **The m1 ranges separate by 0.12 s.** Non-overlapping, but thin enough that a twelfth repetition landing badly would close it. Read it as *roughly a third*, not as 37.0%.
...
- that +37.0% is *the* cost of determinism rather than the cost on one configuration, on one pipeline, at one size.
